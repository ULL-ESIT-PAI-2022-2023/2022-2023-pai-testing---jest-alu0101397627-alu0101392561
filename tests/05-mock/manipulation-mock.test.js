...
const RAMDOM_NUMBER_EXPECTED = 0.123456789 /// we create a possible result of calling the function

beforeEach(() => {
  // In the before each, we use the funcionalities of the mocks function to emulate the random number using jest.spy on
  jest.spyOn(global.Math, 'random').mockReturnValue(RAMDOM_NUMBER_EXPECTED);
});
...
test('it should return a random value', () => {
  ...
  expect(getRandomNumber()).toBe(RAMDOM_NUMBER_EXPECTED);
});